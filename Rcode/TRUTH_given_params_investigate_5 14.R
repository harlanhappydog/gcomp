...
print(c(sumtheta00,  sumtheta11))
# If these were continuous, we would compute theta00 and theta11 by Monte Carlo

nbig<-200
C1sim<-rbinom(nbig,1,C1prob(c1=1))
# Simulating under X1=1, X2=1 intervention
Y1sim<-rep(0,nbig)
for(ii in 1:nbig){Y1sim[ii]<-rbinom(1,1, Y1prob(y1=1,c1= C1sim[ii],x1=1))}
C2sim<-rep(0,nbig)
for(ii in 1:nbig){C2sim[ii]<-rbinom(1,1, C2prob(c2=1,c1=C1sim[ii],x1=1))}
Y2sim<-rep(0,nbig)
for(ii in 1:nbig){Y2sim[ii]<-rbinom(1,1, Y2prob(y2=1,c2=C2sim[ii],x2=1,y1=Y1sim[ii]))}
theta11_sim<-(Y2sim)

# Simulating under X1=0, X2=0 intervention
Y1sim<-rep(0,nbig)
for(ii in 1:nbig){Y1sim[ii]<-rbinom(1,1, Y1prob(y1=1,c1= C1sim[ii],x1=0))}
C2sim<-rep(0,nbig)
for(ii in 1:nbig){C2sim[ii]<-rbinom(1,1, C2prob(c2=1,c1=C1sim[ii],x1=0))}
Y2sim<-rep(0,nbig)
for(ii in 1:nbig){Y2sim[ii]<-rbinom(1,1, Y2prob(y2=1,c2=C2sim[ii],x2=0,y1=Y1sim[ii]))}
theta00_sim<-(Y2sim)


print(c(mean(sumtheta00Bvec_Y1_wH ), mean(theta00_sim), sumtheta00))
print(c(mean(sumtheta11Bvec_Y1_wH ), mean(theta11_sim), sumtheta11))

Fpval_Y1_wH <- 1 - mean((theta11_sim - theta00_sim) >= 0)
Bpval_Y1_wH <- 1 - mean((sumtheta11Bvec_Y1_wH - sumtheta00Bvec_Y1_wH) > 0)

print(c(Fpval_Y1_wH, Bpval_Y1_wH))
