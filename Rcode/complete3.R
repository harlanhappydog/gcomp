...
errorcol<-function(true_gammas, colname="simgamma_wY1_wH"){
for(i in 1:length(true_gammas)){		
dat <-na.omit(true_gammas[[i]][,c("H", "C1", "C2", "X1", "X2", "Y1", "Y2", "Y2avg","cond",  "gamma", colname)])
		
if(i==1){
	dat_check<-data.frame(dat[!duplicated(dat[,c(colname)]),])
		e<-checkmat (dat[!duplicated(dat[,c(colname)]),])
colnames(e)<-paste("error",i,sep="_")
	dat_check <-data.frame(dat_check[,!colnames(dat_check)==c(colname)],e)
	}
if(i>1){
	e<-checkmat (dat[!duplicated(dat[,c(colname)]),])
colnames(e)<-paste("error",i,sep="_")
...
Error_List<-list(E_wY1_wH=errorcol(true_gammas,"simgamma_wY1_wH"),
E_Y1_wH=errorcol(true_gammas,"simgamma_Y1_wH"),
E_Y1_H=errorcol(true_gammas,"simgamma_Y1_H"),
E_wY1_H=errorcol(true_gammas,"simgamma_wY1_H"))



######################################################################## 
# G_wY1_wH <-na.omit(true_gammas[[i]][,c("H", "C1", "C2", "X1", "X2", "Y1", "Y2", "Y2avg","cond",  "gamma", "simgamma_wY1_wH")])
# G_Y1_wH <-na.omit(true_gammas[[i]][,c("H", "C1", "C2", "X1", "X2", "Y1", "Y2", "Y2avg","cond",  "gamma", "simgamma_Y1_wH")])
# G_Y1_H <-na.omit(true_gammas[[i]][,c("H", "C1", "C2", "X1", "X2", "Y1", "Y2", "Y2avg","cond",  "gamma", "simgamma_Y1_H")])
# G_wY1_H <-na.omit(true_gammas[[i]][,c("H", "C1", "C2", "X1", "X2", "Y1", "Y2", "Y2avg","cond",  "gamma", "simgamma_wY1_H")])

# errorList<-list(E_wY1_wH=errorcol(G_wY1_wH,"simgamma_wY1_wH"),
# E_Y1_wH=errorcol(G_Y1_wH,"simgamma_Y1_wH"),
# E_Y1_H=errorcol(G_Y1_H,"simgamma_Y1_H"),
# E_wY1_H=errorcol(G_wY1_H,"simgamma_wY1_H"))
...
err_summary<-list()
for(j in 1:4){
start<-dim(Error_List[[j]])[2]-dim(dsgn)[1]+1
err_summary[[j]]<-data.frame(Error_List[[j]][,1:(start-1)],	"avg_err"= rowMeans(Error_List[[j]][,start:dim(Error_List[[j]])[2]]))
}
names(err_summary)<-names(Error_List)
power_summary<- colMeans(power)

save(err_summary, file="~/results/err_summary_n10000.rda")
save(power_summary, file="~/results/power_summary_n10000.rda")

save(Error_List, file="~/results/errorList_n10000.rda")
save(true_gammas, file="~/results/true_gammas_n10000.rda")
save(Bayesresult, file="~/results/Bayesresult_n10000.rda")
save(power, file="~/results/power_n10000.rda")
