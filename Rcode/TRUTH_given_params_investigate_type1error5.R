...
require(gridExtra)
plot1 <- qplot((dsgn[, 5][diff==0]),    nullpower[, 2], col = as.factor(dsgn[, 2][diff==0]), shape=as.factor(dsgn[, 6][diff==0]), ylim=c(0,0.5))+geom_abline(slope=0,intercept=alpha)+  geom_jitter(width = 0.2, height = 0)+ggtitle("Type 1 error: g-comp|H without Y1")
plot2 <- qplot((dsgn[, 5][diff==0]),    nullpower[, 4], col = as.factor(dsgn[, 2][diff==0]), shape=as.factor(dsgn[, 6][diff==0]), ylim=c(0,0.5))+geom_abline(slope=0,intercept=alpha)+  geom_jitter(width = 0.2, height = 0)+ggtitle("Type 1 error: g-comp with Y1")
grid.arrange(plot1, plot2, ncol=1)
