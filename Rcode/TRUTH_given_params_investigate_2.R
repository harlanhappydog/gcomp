    ...
    for (c2i in 0:1) {
        for (y1i in 0:1) {
            for (c1i in 0:1) {
                ...
                # P(C1=1 or =0)
                p1<-interceptc
                pc1<-(1/(1+exp(-(p1/abs(max(c(0.5,0.5*p1)))))))
                aa <- (as.numeric(c1i == 1) * (pc1) + as.numeric(c1i == 0) * (1-pc1))

                # P(Y1=1 or =0|C1,X1)
                p1<-(intercepty + dsgn[i, 5]*c1i + dsgn[i, 3]*myX1 + dsgn[i, 8]*myH)
                py1<-(1/(1+exp(-(p1/abs(max(c(0.5,0.5*p1)))))))
                bb <- as.numeric(y1i == 1) * (py1) + as.numeric(y1i == 0) * (1 - py1)

                # P(C2=1 or =0|C1,X1)
                p1<-(interceptc + dsgn[i, 4]*c1i + dsgn[i, 6]*myX1)
                pc2<-(1/(1+exp(-(p1/abs(max(c(0.5,0.5*p1)))))))
                cc <- as.numeric(c2i == 1) * (pc2)  + as.numeric(c2i == 0)*(1-pc2)
                
                # P(Y2=1|C2,X2,Y1)
                p1<-intercepty+dsgn[i, 9]*myZ + dsgn[i, 5]*c2i + dsgn[i, 3]*myX2 + dsgn[i, 8]*myH
                py2<-(1/(1+exp(-(p1/abs(max(c(0.5,0.5*p1)))))))
                dd <- (as.numeric(y1i == 1) * 1 + as.numeric(y1i == 0) * (py2)     )           
...
#        set.seed(seedlist[j])
		...
		p1<-intercepth
        H <-rbinom(n, 1, (1/(1+exp(-(p1/abs(max(c(0.5,0.5*p1))))))) )
		p1<-interceptz + dsgn[i, 10]*H
        Z <-rbinom(n, 1, (1/(1+exp(-(p1/abs(max(c(0.5,0.5*p1))))))) )
		p1<-interceptc
        C1 <-rbinom(n, 1, (1/(1+exp(-(p1/abs(max(c(0.5,0.5*p1))))))) )
		p1<-interceptx + dsgn[i, 1]*C1 + dsgn[i, 2]*H
        X1<- rbinom(n, 1, (1/(1+exp(-(p1/abs(max(c(0.5,0.5*p1))))))) )
		p1<-interceptc + dsgn[i, 4]*C1 + dsgn[i, 6]*X1
        C2 <-rbinom(n, 1, (1/(1+exp(-(p1/abs(max(c(0.5,0.5*p1))))))) )
        p1<-intercepty + dsgn[i, 5]*C1 + dsgn[i, 3]*X1 + dsgn[i, 8]*H         
        Y1 <-rbinom(n, 1, (1/(1+exp(-(p1/abs(max(c(0.5,0.5*p1))))))) )
		p1<-interceptx + dsgn[i, 1]*C2  + dsgn[i, 2]*H + dsgn[i, 7]*Y1 + dsgn[i, 9]*Z + dsgn[i, 11]*X1
        X2 <-rbinom(n, 1, (1/(1+exp(-(p1/abs(max(c(0.5,0.5*p1))))))) )
        Y2 <- rep(1, n)  
        p1<-intercepty + dsgn[i, 9]*Z + dsgn[i, 5]*C2 + dsgn[i, 3]*X2 + dsgn[i, 8]*H
        Y2[Y1 == 0]<-rbinom(length(Y2[Y1 == 0]), 1, (1/(1+exp(-(1/(1+exp(-(p1/abs(max(c(0.5,0.5*p1)))))))))) )     
