...
numsim <- 30  												### number of datasets generated under each scenario
n <- 5000														### size of each dataset
set.seed(13)  													### for reproducibility
nB<-100															### number of MC samples
...
checkmat<-function(mat){
	data.frame(mat, "error"=round(apply(cbind(abs(mat[,dim(mat)[2]]-mat$gamma),	abs(mat[,dim(mat)[2]]-mat$Y2avg)),1, min),2))
...
G_wY1_wH <-na.omit(true_gammas[[1]][,c("H", "C1", "C2", "X1", "X2", "Y1", "Y2", "Y2avg","cond",  "gamma", "simgamma_wY1_wH")])
checkmat (G_wY1_wH[!duplicated(G_wY1_wH[,c("simgamma_wY1_wH")]),])

G_Y1_wH <-na.omit(true_gammas[[1]][,c("H", "C1", "C2", "X1", "X2", "Y1", "Y2", "Y2avg","cond",  "gamma", "simgamma_Y1_wH")])
checkmat (G_Y1_wH[!duplicated(G_Y1_wH[,c("simgamma_Y1_wH")]),])

G_Y1_H <-na.omit(true_gammas[[1]][,c("H", "C1", "C2", "X1", "X2", "Y1", "Y2", "Y2avg","cond",  "gamma", "simgamma_Y1_H")])
checkmat (G_Y1_H[!duplicated(G_Y1_H[,c("simgamma_Y1_H")]),])

G_wY1_H <-na.omit(true_gammas[[1]][,c("H", "C1", "C2", "X1", "X2", "Y1", "Y2", "Y2avg","cond",  "gamma", "simgamma_wY1_H")])
checkmat (G_wY1_H[!duplicated(G_wY1_H[,c("simgamma_wY1_H")]),])
