...
nB<-200												### number of MC samples
Bayesresult <- matrix(0, dim(dsgn)[1] * numsim, 5)
...
# Obtain estimates for all gammas:
y2i<-1
 for (x2i in 0:1) {
x1i<-x2i
    for (y1i in 0:1) {
...
# Use estimates of gammas to cauculate estimated effect difference:
sumtheta11B<-0
sumtheta00B<-0
y2i<-1
for(x1i in 0:1){        
...
# calculate one-sided p-values:
         ...
         Bayesresult[kkk, ] <- c(i, j, diff_theta[i], Bpval_Y1_wH, Hpval)
...
Error_List<-list(E_Y1_wH=errorcol(true_gammas,"simgamma_Y1_wH"))


######################################################################## 
alpha<-0.1
power <- matrix(0, max(ilist), 4)
for (i in ilist) {
    power[i, ] <- c(i, 
...
colnames(power)<-c("i","diff_theta","with Y1 without H", "IV")


err_summary<-list()
for(j in 1){
start<-dim(Error_List[[j]])[2]-length(ilist)+1
err_summary[[j]]<-data.frame(Error_List[[j]][,1:(start-1)],	"avg_err"= (Error_List[[j]][,start]))
if(length(start:dim(Error_List[[j]])[2])>1){
...
save(err_summary, file=paste("~/results/err_summary",n, nB, numsim,ilist,".rda",sep="_"))
save(power_summary, file=paste("~/results/power_summary",n, nB, numsim,ilist,".rda",sep="_"))

save(Error_List, file=paste("~/results/Error_List",n, nB, numsim,ilist,".rda",sep="_"))
save(true_gammas, file=paste("~/results/true_gammas",n, nB, numsim,ilist,".rda",sep="_"))
save(Bayesresult, file=paste("~/results/Bayesresult",n, nB, numsim,ilist,".rda",sep="_"))
save(power, file=paste("~/results/power",n, nB, numsim,ilist,".rda",sep="_"))
