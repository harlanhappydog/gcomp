...
fixlev<-c(0.5, 1)  # levels for not interesting parameters
...
qplot(matt[,2],matt[,1],  color=as.factor(dsgn[,5]), shape=as.factor(dsgn[,6]), ylim=c(0,1), xlim=c(0,1))+geom_abline(slope=1,intercept=0)
qplot(matt[,2],matt[,1],  color=as.factor(dsgn[,3]), shape=as.factor(dsgn[,6]), ylim=c(0,1), xlim=c(0,1))+geom_abline(slope=1,intercept=0)
