...
Bayesresult <- matrix(0, dim(dsgn)[1] * numsim, 10)
...
######################################################################## 
# ## BAYESIAN APPROACH WITHOUT Y1 (uniform priors):
...
######################################################################## 
# ## BAYESIAN APPROACH WITHOUT Y1 (uniform priors) MODIFIED
        HprobB <- function(h, nm = 1) {
            ss <- sum(simdata$H)
            nn <- length(simdata$H)
            pp <- (rbeta(nm, (ss + 1), (nn - ss + 1)))
        ...
        Y2probB <- function(h, y2, c2, x2, nm = 1) {
            ss <- sum(simdata$Y2[simdata$H == h & simdata$C2 == c2 & simdata$X2 == x2])
            nn <- length(simdata$Y2[simdata$H == h & simdata$C2 == c2 & simdata$X2 == x2])
            pp <- (rbeta(nm, (ss + 1), (nn - ss + 1)))
            if (y2 == 0) {
                return(1 - as.numeric(pp))
            }
        ...
        for (c2i in 0:1) {
            for (c1i in 0:1) {
                for (hi in 0:1) {
                  ...
                  sumtheta11B <- sumtheta11B + HprobB(h = hi, nB) * C1probB(h = hi, c1 = c1i, nm = nB) * 
                  C2probB(h = hi,   c2 = c2i, c1 = c1i, x1 = 1, nm = nB) * 
                  Y2probB(h = hi, y2 = 1, c2 = c2i,  x2 = 1, nm = nB)
                  
                  sumtheta00B <- sumtheta00B + HprobB(h = hi, nB) * C1probB(h = hi, c1 = c1i, nm = nB) * C2probB(h = hi, 
                    c2 = c2i, c1 = c1i, x1 = 0, nm = nB) *
                     Y2probB(h = hi, y2 = 1, c2 = c2i, x2 = 0, nm = nB)
                  
                }
            }
        }
        
        sumtheta00BvecMOD <- sumtheta00B
        sumtheta11BvecMOD <- sumtheta11B
...
# ################################################################
# ## BAYESIAN APPROACH WITH Y1 (uniform priors):
...
for(y1i in 0:1){
for(c1i in 0:1){
sumtheta11B<-sumtheta11B+C1probB(c1=c1i,nm=nB)*Y1probB(y1=y1i,c1=c1i,x1=1,nm=nB)*C2probB(c2=c2i,c1=c1i,x1=1,nm=nB)*Y2probB(y2=1,c2=c2i,x2=1,y1=y1i,nm=nB)
sumtheta00B<-sumtheta00B +C1probB(c1=c1i,nm=nB)*Y1probB(y1=y1i,c1=c1i,x1=0,nm=nB)*C2probB(c2=c2i,c1=c1i,x1=0,nm=nB)*Y2probB(y2=1,c2=c2i,x2=0,y1=y1i,nm=nB)
}}}

sumtheta00BvecY1<-sumtheta00B
sumtheta11BvecY1<-sumtheta11B
...
#one-sided p-values:
        Bpval <- 1 - mean((sumtheta11Bvec - sumtheta00Bvec) > 0)
        BpvalY1<-1-mean((sumtheta11BvecY1-sumtheta00BvecY1)>0)
        BpvalMOD<-1-mean((sumtheta11BvecMOD-sumtheta00BvecMOD)>0)
        Hpval <- (1-pnorm((summary(glm(Y2 ~ H, data = simdata, family = "binomial"))$coefficients[2, 3])))
        print(Bpval)
        print(Hpval)
        Bayesresult[kkk, ] <- c(i, j, c(1, 1), mean(sumtheta00Bvec), mean(sumtheta11Bvec), Bpval, Hpval, BpvalY1, BpvalMOD)
        kkk <- kkk + 1
    }
}
...
power <- matrix(0, dim(dsgn)[1], 4)
for (i in (dim(dsgn)[1]:1)) {
    power[i, ] <- c(i, mean(Bayesresult[Bayesresult[, 1] == i, ][, 7] <= alpha), mean(Bayesresult[Bayesresult[, 
        1] == i, ][, 8] <= alpha), mean(Bayesresult[Bayesresult[,1] == i, ][, 9] <= alpha))
}
...
require(gridExtra)
plot1 <- qplot(diff,    power[, 2], col = as.factor(dsgn[, 3]), shape=as.factor(dsgn[, 5]), ylim=c(0,1))+geom_abline(slope=0,intercept=alpha)+  geom_jitter(width = 0.05, height = 0)+ggtitle("Power: g-comp|H")
plot2 <- qplot(diff,    power[, 3], col = as.factor(dsgn[, 2]), shape=as.factor(dsgn[, 3]), ylim=c(0,1))+geom_abline(slope=0,intercept=alpha)+  geom_jitter(width = 0.05, height = 0)+ggtitle("Power: Instrumental Variable")
grid.arrange(plot1, plot2, ncol=1) 

nullpower<-power[diff==0,]



require(gridExtra)
plot1 <- qplot((dsgn[, 5][diff==0]),    nullpower[, 2], col = as.factor(dsgn[, 1][diff==0]), shape=as.factor(dsgn[, 6][diff==0]), ylim=c(0,1))+geom_abline(slope=0,intercept=alpha)+  geom_jitter(width = 0.2, height = 0)
plot2 <- qplot((dsgn[, 5][diff==0]),    nullpower[, 3], col = as.factor(dsgn[, 1][diff==0]), shape=as.factor(dsgn[, 6][diff==0]), ylim=c(0,1))+geom_abline(slope=0,intercept=alpha)+  geom_jitter(width = 0.2, height = 0)
grid.arrange(plot1, plot2, ncol=1)


# nullpower1 <-nullpower
# dsgn1<-dsgn
