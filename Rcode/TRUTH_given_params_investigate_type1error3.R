...
numsim <- 300  ### number of datasets generated under each scenario
...
colnames(Bayesresult)<-c("i","j","a1","a1","mean00","mean11","withoutY1","IV","withY1","MOD")
power <- matrix(0, dim(dsgn)[1], 5)
for (i in (dim(dsgn)[1]:1)) {
    power[i, ] <- c(i, mean(Bayesresult[Bayesresult[, 1] == i, ][, 7] <= alpha), mean(Bayesresult[Bayesresult[, 
        1] == i, ][, 8] <= alpha), mean(Bayesresult[Bayesresult[,1] == i, ][, 9] <= alpha), mean(Bayesresult[Bayesresult[,1] == i, ][, 10] <= alpha))
}
...
require(gridExtra)
plot1 <- qplot((dsgn[, 5][diff==0]),    nullpower[, 2], col = as.factor(dsgn[, 1][diff==0]), shape=as.factor(dsgn[, 6][diff==0]), ylim=c(0,1))+geom_abline(slope=0,intercept=alpha)+  geom_jitter(width = 0.2, height = 0)+ggtitle("Type 1 error: g-comp|H without Y1")
plot2 <- qplot((dsgn[, 5][diff==0]),    nullpower[, 4], col = as.factor(dsgn[, 1][diff==0]), shape=as.factor(dsgn[, 6][diff==0]), ylim=c(0,1))+geom_abline(slope=0,intercept=alpha)+  geom_jitter(width = 0.2, height = 0)+ggtitle("Type 1 error: g-comp|H with Y1")
grid.arrange(plot1, plot2, ncol=1)


# nullpower2 <-nullpower
# dsgn2<-dsgn
