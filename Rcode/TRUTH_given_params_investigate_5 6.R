...
C1probB_0<-C1probB(0)
        ...
        C2probB <- function(c2, c1, x1, nm = 10) {
...
C2probB_11x1<-C2probB(1,1,1)
C2probB_11x0<-C2probB(1,1,0)
C2probB_10x1<-C2probB(1,0,1)
C2probB_10x0<-C2probB(1,0,0)
C2probB_01x1<-C2probB(0,1,1)
C2probB_01x0<-C2probB(0,1,0)
C2probB_00x1<-C2probB(0,0,1)
C2probB_00x0<-C2probB(0,0,0)
...
Y2probB_111_x11<-Y2probB( 1, 1, 1, 1, 1)
...
# ## BAYESIAN APPROACH WITHOUT Y1 WITH H  (uniform priors):
 ...
 sumtheta11B <- rep(0,200)
  sumtheta00B <- rep(0,200)
        for(hh in 1:200){
        nB <- 1
        sumtheta11B[hh] <- 0
        sumtheta00B[hh] <- 0
        for (c2i in 0:1) {
            for (c1i in 0:1) {
                for (hi in 0:1) {
                  ...
                  print(apply(cbind(c(HprobB(h = hi, nB) , C1probB(h = hi, c1 = c1i, nm = nB) ,
                  C2probB(h = hi,   c2 = c2i, c1 = c1i, x1 = 1, nm = nB) , 
                  Y2probB(h = hi, y2 = 1, c1 = c1i, c2 = c2i, x1 = 1,  x2 = 1, nm = nB))),2,mean))
                 
                  sumtheta11B[hh] <- sumtheta11B[hh] + HprobB(h = hi, nB) * C1probB(h = hi, c1 = c1i, nm = nB) * 
                  C2probB(h = hi,   c2 = c2i, c1 = c1i, x1 = 1, nm = nB) * 
                  Y2probB(h = hi, y2 = 1, c1 = c1i, c2 = c2i, x1 = 1,  x2 = 1, nm = nB)
                  
                  sumtheta00B[hh] <- sumtheta00B[hh] + HprobB(h = hi, nB) * C1probB(h = hi, c1 = c1i, nm = nB) * C2probB(h = hi, 
...
1 - mean((sumtheta11B - sumtheta00B) > 0)
summary(glm(Y2~X2+X1+H+C1+C2,data=simdata, family='binomial'))
# ################################################################
