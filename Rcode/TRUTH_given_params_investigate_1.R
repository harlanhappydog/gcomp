...
#        set.seed(seedlist[j])
		...
		p1<-intercepth
        H <-rbinom(n, 1, (1/(1+exp(-(p1/abs(max(p1)))))) )
		p1<-interceptz + dsgn[i, 10]*H
        Z <-rbinom(n, 1, (1/(1+exp(-(p1/abs(max(p1)))))) )
		p1<-interceptc
        C1 <-rbinom(n, 1, (1/(1+exp(-(p1/abs(max(p1)))))) )
		p1<-interceptx + dsgn[i, 1]*C1 + dsgn[i, 2]*H
        X1<- rbinom(n, 1, (1/(1+exp(-(p1/abs(max(p1)))))) )
		p1<-interceptc + dsgn[i, 4]*C1 + dsgn[i, 6]*X1
        C2 <-rbinom(n, 1, (1/(1+exp(-(p1/abs(max(p1)))))) )
        p1<-intercepty + dsgn[i, 5]*C1 + dsgn[i, 3]*X1 + dsgn[i, 8]*H         
        Y1 <-rbinom(n, 1, (1/(1+exp(-(p1/abs(max(p1)))))) )
		p1<-interceptx + dsgn[i, 1]*C2  + dsgn[i, 2]*H + dsgn[i, 7]*Y1 + dsgn[i, 9]*Z + dsgn[i, 11]*X1
        X2 <-rbinom(n, 1, (1/(1+exp(-(p1/abs(max(p1)))))) )
        Y2 <- rep(1, n)  
        p1<-intercepty + dsgn[i, 9]*Z + dsgn[i, 5]*C2 + dsgn[i, 3]*X2 + dsgn[i, 8]*H
        Y2[Y1 == 0]<-rbinom(length(Y2[Y1 == 0]), 1, (1/(1+exp(-(1/(1+exp(-(p1/abs(max(p1))))))))) )     
...
# ## BAYESIAN APPROACH:  WITHOUT Y1, WITHOUT H (uniform priors):
        ...
        Y2probB <- function( y2, c1, c2, x1, x2, nm = 1) {
            ...
            nn <- length(simdata$Y2[ simdata$C2 == c2 & simdata$X2 == x2 & simdata$C1 == c1 & 
                simdata$X1 == x1])
            pp <- (rbeta(nm, (ss + 1), (nn - ss + 1)))
            if (y2 == 0) {
                return(1 - as.numeric(pp))
...
require(gridExtra)
plot1 <- qplot(diff_theta,    power[, 3], col = as.factor(dsgn[, 3]), shape=as.factor(dsgn[, 5]), ylim=c(0,1))+geom_abline(slope=0,intercept=alpha)+  geom_jitter(width = 0.05, height = 0)+ggtitle(paste(colnames(power)[3]))
plot2 <- qplot(diff_theta,    power[, 4], col = as.factor(dsgn[, 2]), shape=as.factor(dsgn[, 3]), ylim=c(0,1))+geom_abline(slope=0,intercept=alpha)+  geom_jitter(width = 0.05, height = 0)+ggtitle(paste(colnames(power)[4]))
grid.arrange(plot1, plot2, ncol=1) 
...
require(gridExtra)
plot1 <- qplot((dsgn[, 5][diff_theta==0]),    nullpower[, 5], col = as.factor(dsgn[, 11][diff_theta==0]), shape=as.factor(dsgn[, 6][diff_theta==0]), ylim=c(0,0.5))+geom_abline(slope=0,intercept=alpha)+  geom_jitter(width = 0.2, height = 0)+ggtitle(paste(colnames(power)[5]))
plot2 <- qplot((dsgn[, 5][diff_theta==0]),    nullpower[, 6], col = as.factor(dsgn[, 11][diff_theta==0]), shape=as.factor(dsgn[, 6][diff_theta==0]), ylim=c(0,0.5))+geom_abline(slope=0,intercept=alpha)+  geom_jitter(width = 0.2, height = 0)+ggtitle(paste(colnames(power)[6]))
grid.arrange(plot1, plot2, ncol=1)
