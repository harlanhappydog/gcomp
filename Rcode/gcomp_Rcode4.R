    ...
    for (c2i in 0:1) {
        for (y1i in 0:1) {
            for (c1i in 0:1) {
                ...
                # P(Y1=1 or =0|C1,X1)
                py1<-1/(1+exp(-(intercept + dsgn[i, 5]*c1i + dsgn[i, 3]*myX1 + dsgn[i, 8]*myH)))
                ...
                # P(Y2=1|C2,X2,Y1)
                dd <- (as.numeric(y1i == 1) * 1 + as.numeric(y1i == 0) * ( (1/(1+exp(-(intercept + dsgn[i, 9]*myZ + dsgn[i, 5]*c2i + dsgn[i, 3]*myX2 + dsgn[i, 8]*myH)))) ))                
...
qplot(dsgn[,3], diff,  color=as.factor(dsgn[,6]), shape=as.factor(dsgn[,5]))
...
qplot(dsgn[dsgn[,5]==1,][,6], diff[dsgn[,5]==1],  color=as.factor(dsgn[dsgn[,5]==1,][,3]))
qplot(dsgn[dsgn[,6]==1,][,5], diff[dsgn[,6]==1],  color=as.factor(dsgn[dsgn[,6]==1,][,3]))
