...
seqlev<-c(0,1,2,3,4) # levels for  interesting parameters
...
Bayesresult <- matrix(0, dim(dsgn)[1] * numsim, 9)
n <- 200  ### size of each dataset
set.seed(13)  ### for reproducibility
seedlist <- sample(1:250000, size = numsim)
savecor<-matrix(0,numsim*dim(dsgn)[1],5)
...
print("correlation")
savecor[kkk,]<-c(i,j,dsgn[i, 2],dsgn[i, 3],cor(simdata$Y2,simdata$H))
######################################################################## 
...
#one-sided p-values:
        ...
        print(Bpval)
        print(Hpval)
        Bayesresult[kkk, ] <- c(i, j, c(1, 1), mean(sumtheta00Bvec), mean(sumtheta11Bvec), Bpval, Hpval, BpvalY1)
        kkk <- kkk + 1
    }
}
...
require(gridExtra)
plot1 <- qplot(diff,    power[, 4], col = as.factor(dsgn[, 3]), shape=as.factor(dsgn[, 11]), ylim=c(0,1))+geom_abline(slope=0,intercept=alpha)+  geom_jitter(width = 0.05, height = 0)+ggtitle("Power: g-comp|H with Y1")
plot2 <- qplot(diff,    power[, 3], col = as.factor(dsgn[, 2]), shape=as.factor(dsgn[, 11]), ylim=c(0,1))+geom_abline(slope=0,intercept=alpha)+  geom_jitter(width = 0.05, height = 0)+ggtitle("Power: Instrumental Variable")
grid.arrange(plot1, plot2, ncol=1) 

nullpower<-power[diff==0,]

qplot(savecor[,3],savecor[,5],col=as.factor(savecor[,4]), shape=as.factor(rep(dsgn[, 11],numsim)))+  geom_jitter(width = 1, height = 0.01)+ggtitle("Correlation: H and Y2")


#require(gridExtra)
#plot1 <- qplot((dsgn[, 5][diff==0]),    nullpower[, 2], col = as.factor(dsgn[, 4][diff==0]), shape=as.factor(dsgn[, 6][diff==0]), ylim=c(0,1))+geom_abline(slope=0,intercept=alpha)+  geom_jitter(width = 0.2, height = 0)
#plot2 <- qplot((dsgn[, 5][diff==0]),    nullpower[, 3], col = as.factor(dsgn[, 4][diff==0]), shape=as.factor(dsgn[, 6][diff==0]), ylim=c(0,1))+geom_abline(slope=0,intercept=alpha)+  geom_jitter(width = 0.2, height = 0)
#grid.arrange(plot1, plot2, ncol=1)
