...
as.matrix(compardat[compardat$DDS17_admission.x>5,])
...
compardat<-merge(newdat, olddat,by="PATID")

mean(compardat$treat.x==compardat$treat.y)
mean(compardat$a1c_adm.x ==compardat$a1c_adm.y)

cbind(compardat$a1c_dis.x,compardat$a1c_dis.y)

mean(compardat$DDS17_admission.x ==compardat$DDS17_admission.y)
########################################
#### A1C Change:
library(psych)
