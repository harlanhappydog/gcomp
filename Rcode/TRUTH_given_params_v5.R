...
require(gridExtra)
plot1 <- qplot(diff,    power[, 2], col = as.factor(dsgn[, 2]), shape=as.factor(dsgn[, 5]), ylim=c(0,1))+geom_abline(slope=0,intercept=alpha)
plot2 <- qplot(diff,    power[, 3], col = as.factor(dsgn[, 2]), shape=as.factor(dsgn[, 5]), ylim=c(0,1))+geom_abline(slope=0,intercept=alpha)
