    ...
    for (c2i in 0:1) {
        for (y1i in 0:1) {
            for (c1i in 0:1) {
                ...
                # P(Y1=1 or =0|C1,X1)
                py1<-1/(1+exp(-(intercepty + dsgn[i, 5] + dsgn[i, 3]*myX1 + dsgn[i, 8]*myH)))
...
qplot(dsgn[,6], diff_theta,  color=as.factor(dsgn[,5]), shape=as.factor(dsgn[,4]))
...
#        set.seed(seedlist[j])
        ...
        Y1 <-rbinom(n, 1, (1/(1+exp(-(intercepty + dsgn[i, 3]*X1 + dsgn[i, 8]*H)))) ) 
